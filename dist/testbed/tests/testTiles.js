function TestTiles() {
  ...
  var N = 200;
  var M = 10;
  var position = new liquidfun.b2Vec2;
  position.y = 0.0;
  for (var j = 0; j < M; ++j) {
    position.x = -N * a;
    for (var i = 0; i < N; ++i) {
  ...
  var x = new liquidfun.b2Vec2(-7.0, 0.75);
  var y = new liquidfun.b2Vec2;
  var deltaX = new liquidfun.b2Vec2(0.5625, 1.25);
  var deltaY = new liquidfun.b2Vec2(1.125, 0.0);
  ...
  for (var i = 0; i < count; ++i) {
    ...
    for (j = i; j < count; ++j) {
      ...
      ++this.fixtureCount;
      liquidfun.b2Vec2.Add(y, y, deltaY);
    }
    liquidfun.b2Vec2.Add(x, x, deltaX);
  }
}